...
(async function App() {
    ...
    try {
        ...
        const errorMsg = i18n.getLanguage() === 'he' ? 'שגיאה בטעינת המתכונים' : 'Error loading recipes';
        const retryMsg = i18n.getLanguage() === 'he' ? 'נסו שוב' : 'Try again';
    ...
    function updateStaticText() {
        ...
        // Update hero title - preserve the structure with two parts
        ...
        if (footerTagline) {
            const heartIcon = '<span class="text-fr-primary mx-1"><i class="fas fa-heart"></i></span>';
            footerTagline.innerHTML = `${i18n.t('footer.tagline')} ${heartIcon}`;
        }

        // Update "Back to Hub" link text
    ...
    document.getElementById('clear-filters-btn').addEventListener('click', () => {
        searchInput.value = '';
        searchClear.classList.add('hidden');
        const allLabel = i18n.getLanguage() === 'he' ? 'הכל' : 'All';
        RecipeData.filterRecipes('', allLabel);
        renderHome();
    });
